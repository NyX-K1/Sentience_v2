...
export function Component() {
    ...
    useEffect(() => {
        ...
        const slides = [
            { title: "Ethereal Glow", description: "A soft, radiant light that illuminates the soul.", media: "https://images.unsplash.com/photo-1542401886-65d6c61db217?q=80&w=2070&auto=format&fit=crop" },
            { title: "Rose Mirage", description: "Lost in a desert of blooming dreams and endless horizons.", media: "https://images.unsplash.com/photo-1516035069371-29a1b244cc32?q=80&w=2000&auto=format&fit=crop" },
            { title: "Velvet Mystique", description: "Wrapped in the deep, luxurious embrace of the night.", media: "https://images.unsplash.com/photo-1493246507139-91e8fad9978e?q=80&w=2070&auto=format&fit=crop" },
            { title: "Golden Hour", description: "That fleeting moment when the world is dipped in gold.", media: "https://images.unsplash.com/photo-1500382017468-9049fed747ef?q=80&w=2000&auto=format&fit=crop" },
            { title: "Midnight Dreams", description: "Where reality fades and imagination takes flight.", media: "https://images.unsplash.com/photo-1504198458649-3128b932f49e?q=80&w=2000&auto=format&fit=crop" },
            { title: "Silver Light", description: "A cool, metallic shimmer reflecting the urban pulse.", media: "https://images.unsplash.com/photo-1510915361894-3a6521431535?q=80&w=2070&auto=format&fit=crop" }
        ];
    ...
    `;
        ...
        const navigateToSlide = (targetIndex: number) => {
            ...
            if (!slideTextures[targetIndex]) return;
        ...
        const initRenderer = async () => {
            ...
            for (const s of slides) {
                try {
                    ...
                    if (!isMountedRef.current) return;
                    console.warn("Failed texture, using fallback");
                    // Create fallback texture
                    const placeholder = new THREE.TextureLoader().load('https://placehold.co/100x100/000000/FFF.png');
                    placeholder.userData = { size: new THREE.Vector2(100, 100) };
                    slideTextures.push(placeholder);
                }
            }
            ...
            const render = () => {
                if (!isMountedRef.current || !renderer) return;
        ...
        return () => {
            ...
            const titleEl = document.getElementById('mainTitle');
            const descEl = document.getElementById('mainDesc');
            if (titleEl) gsap.killTweensOf(titleEl.children);
            if (descEl) gsap.killTweensOf(descEl);
