...
const PRODUCT_DATA: Record<ProductId, ProductData> = {
    left: {
        ...
        description: 'The structural foundation of your psyche. Handles pattern recognition, logical processing, and strategic planning.',
        // Abstract geometric/tech image
        ...
        stats: { connectionStatus: 'Active', batteryLevel: 92 },
        features: [
            { label: 'Processing', value: 98, icon: Cpu },
            { label: 'Latency', value: 4, icon: Zap },
        ],
    },
    right: {
        id: 'right',
        ...
        description: 'The wellspring of imagination and emotion. Governs artistic expression, empathy, and non-linear thought.',
        // Abstract fluid/color image
        ...
        stats: { connectionStatus: 'Synced', batteryLevel: 88 },
        features: [
            { label: 'Resonance', value: 96, icon: Sparkles },
            { label: 'Harmony', value: 94, icon: Music },
        ],
    },
};
            ...
            >
                <AnimatePresence mode="wait">
                    <motion.img
                        ...
                        animate="animate"
                        exit="exit"
                        className="w-full h-full object-cover rounded-full p-1 clip-circle"
                        draggable={false}
                    />
                </AnimatePresence>
            </motion.div>
    ...
    return (
        ...
        >
            ...
            <motion.div variants={ANIMATIONS.item} className="w-full space-y-6 bg-zinc-900/40 p-6 rounded-2xl border border-white/5 backdrop-blur-sm">
                ...
                <div className={`pt-4 flex ${isLeft ? 'justify-start' : 'justify-end'}`}>
                    <button type="button" className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-zinc-300 hover:text-white transition-colors group">
                        <Sliders size={14} /> Neural Analysis
            ...
            <motion.div variants={ANIMATIONS.item} className={`mt-6 flex items-center gap-3 text-zinc-500 ${flexDirClass}`}>
                <Activity size={16} />
                <span className="text-sm font-medium">Synced: {data.stats.batteryLevel}%</span>
            </motion.div>
        </motion.div>
    );
};
