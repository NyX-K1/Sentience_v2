        ...
        >
            ...
            <div className="w-full max-w-xl flex-grow overflow-y-auto scrollbar-hide px-4 pb-32 relative">
                <div className="flex flex-col gap-4">
                    {family === 'Complex' ? (
                        // Group by subFamily for Complex
                        Array.from(new Set(relevantEmotions.map(e => e.subFamily || 'Other'))).map((sub) => {
                            const items = relevantEmotions.filter(e => (e.subFamily || 'Other') === sub);
                            if (items.length === 0) return null;
                            const isExpanded = expandedIntensity === (sub.charCodeAt(0)); // reusing state for grouping

                            return (
                                <div key={sub} className="flex flex-col gap-2">
                                    <button
                                        onClick={() => setExpandedIntensity(isExpanded ? null : (sub.charCodeAt(0)))}
                                    ...
                                    >
                                        <span className="font-semibold text-white/90 tracking-widest uppercase text-sm">
                                            {sub} <span className="text-white/40 ml-2">({items.length})</span>
                                        </span>
                        ...
                        // Group by Intensity for standard families
